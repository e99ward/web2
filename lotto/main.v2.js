...
let number_only = [];
let pred_numbers = [];
const window_size = 24; // = time_seqs
const time_seqs = 480; // = batch_size
...
async function ConvertToTensor(data) {
    ...
    return tf.tidy(() => {
        ...
        values = values.reshape([time_seqs, 1]);
        tf.util.shuffleCombo(inputs, values);
        //const xyDataset = tf.data.zip({inputs, values}).batch(32).shuffle(32);
        const last = flat.size - window_size;
        const latest = flat.slice([last], [window_size]);
        // inputs.print();
        // values.print();
        ...
        // return { xyDataset[0], xyDataset.ys, latest };
    });
}
...
function CreateModel() {
    ...
    model.add(tf.layers.simpleRNN({ inputShape: [, 1], units: 24, returnSequences: true }));
    model.add(tf.layers.simpleRNN({ units: 24, returnSequences: false }));
    model.add(tf.layers.dense({ units: 1 }));

    return model;
}

async function TrainModel(model, inputs, values) {
    // Prepare the model for training.
    // model.compile({ optimizer: 'sgd', loss: 'meanSquaredError' });
    model.compile({
        optimizer: tf.train.sgd(0.00002), //'sgd',
        loss: 'meanSquaredError',
        metrics: ['mse'],
    });
    console.log("OK2: in training");
    //     await model.fit(xs, ys, { epochs: 1000 });
    return await model.fit(inputs, values, {
        batchSize: 32,
        epochs: 200,
        // shuffle: true,
...
async function TestModel(model, inputData) {
    ...
    let data_new = inputData.reshape([1, window_size, 1])
    ...
    for (let k = 0; k < 6; k++) {
        ...
        console.log(preds.mul(22).add(23).print());
